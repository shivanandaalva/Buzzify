...
export default function Headlines({navigation}) {
  ...
  useEffect(() => {
    ...
    await fetch('https://news.google.com/rss/topics/CAAqKggKIiRDQkFTRlFvSUwyMHZNRGx6TVdZU0JXVnVMVWRDR2dKSlRpZ0FQAQ?hl')
    .then((response) => response.text())
  .then((responseData) => rssParser.parse(responseData))
  .then((rss) => {
    setbusiness(rss.items)
    // console.log(rss);
...
fetchData();
  ...
  const data=  business.map((data)=>(data.links.map((e)=>
